...
      
      // Get the message, data, and token directly from the response
      const { message, data, token } = res.data; // <-- CORRECTED LINE

      // Set the message for display
      setMsg(message);

      // This is the crucial fix: directly store the token
      if (token) {
        localStorage.setItem('token', token); // <-- CORRECTED LINE
      }

      // Check for user role and store it
      if (data && data.role) {
